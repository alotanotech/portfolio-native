...
async function publicApi(request, env, pathname) {
    ...
    if (pathname === "/api/projects") {
        const { results } = await env.DB.prepare("SELECT project_index, slug, name, full_name, client, year, type, status, kind, category, description FROM projects WHERE published = 1 ORDER BY project_index, id").all();
...
async function adminApi(request, env, pathname) {
    ...
    if (uploadMatch && request.method === "POST") {
        ...
        if (!CATEGORIES.includes(category)) return error("Invalid image category.");
        ...
        const objectKey = `${decodeURIComponent(uploadMatch[1])}/${crypto.randomUUID()}.${extension}`;
        ...
        const result = await env.DB.prepare("INSERT INTO project_images (project_id, category, object_key, alt_text, sort_order) VALUES (?, ?, ?, ?, ?)")
            .bind(project.id, category, objectKey, String(form.get("alt") || ""), order.next_order).run();
